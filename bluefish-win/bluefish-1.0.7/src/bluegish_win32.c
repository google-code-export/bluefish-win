...
static gchar *return_filename_from_path(Tfilebrowser *filebrowser, GtkTreeModel *store,const GtkTreePath *thispath) {
	...
	while (valid) {
		...
		if (strcmp(name,"\\")==0){
			g_free(name);
			name = g_strdup(getenv("SYSTEMDRIVE"));
		}
		...
		if (!showfulltree && filebrowser->basedir && strcmp(name, filebrowser->basedir) == 0) {
			...
			printf("[3name=%s , retval=%s\n",name,retval);
			tmp = retval;
			if (retval) {
				if (strstr(retval,getenv("SYSTEMDRIVE")) == retval){
					retval = g_strconcat(name, retval+strlen(getenv("SYSTEMDRIVE")),NULL);
				}else{
					retval = g_strconcat(name, "\\", retval,NULL);
				}
			} else {
...
static GList *return_dir_entries(Tfilebrowser *filebrowser,const gchar *dirname) {
	...
	gchar *ondiskname = get_filename_on_disk_encoding(dirname);
	...
	while (GNOME_VFS_OK == gnome_vfs_directory_read_next(handle, gvfi)) {
		...
		if (strcmp(gvfi->name,".")!=0 && strcmp(gvfi->name,"..")!=0) {
			...
			entry->name = get_utf8filename_from_on_disk_encoding(gvfi->name);
...
static GtkTreePath *return_path_from_filename(Tfilebrowser *filebrowser,gchar *this_filename) {
	...
	if (showfulltree) {
		...
		if (root && (strstr(root,getenv("SYSTEMDRIVE"))!=root)) {
...
static GtkTreePath *build_tree_from_path(Tfilebrowser *filebrowser, const gchar *filepath) {
	...
	/* first build path from root to here */
	{
		...
		if (!gtk_tree_model_get_iter_first(GTK_TREE_MODEL(filebrowser->store), &iter)) {
			if (!showfulltree && filebrowser->basedir) {
				...
				iter = add_tree_item(NULL, filebrowser, getenv("SYSTEMDRIVE"), TYPE_DIR, NULL);
		...
		if (prevlen >= totlen) {
			...
			printf("root-->:%s\n",root);
			if (root && (strcmp(root,getenv("SYSTEMDRIVE"))!=0)){
			/* if (root && root[0] != '\\') { */
    ...
    DEBUG_MSG("build_tree_from_path, tmpstr='%s'\n", tmpstr);
			...
			if (!get_iter_by_filename_from_parent(filebrowser->store, &iter, &iter, tmpstr)) {
				if (tmpstr && (strcmp(tmpstr,getenv("SYSTEMDRIVE"))!=0)){
